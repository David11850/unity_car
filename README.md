...
#### 一、云端调度中心 (Cloud Dispatch Center)
**技术栈：** Linux + C++11 + `epoll` 事件驱动模型

**核心职责：**
1. **高并发连接管理**：基于 `epoll` 多路复用，维护海量终端的 TCP 长连接，首包协议认证动态完成异构终端的身份鉴权与会话绑定
2. **全局状态机维护**：实时映射全局拓扑地图与车辆状态树（空闲、配送中、故障等），监控全局运力分布
3. **宏观路径规划与派单 (Global Planning & Dispatch)**：接收订单后，结合车辆当前坐标矩阵，运用图论算法计算全局最优分配策略
4. **航点下发 (Waypoint Assignment)**：将配送任务降维为一系列宏观坐标点（Waypoints），下发至目标车辆，不干预车辆的点间行驶过程

#### 二、边缘计算核心 (Edge Computing Core / Local Planner)
...
#### 三、仿真与感知表现层 (Simulation & Perception Layer)
